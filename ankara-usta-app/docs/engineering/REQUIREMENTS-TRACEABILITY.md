...
| WIZ-003 | All 26 services have runtime-validated service-specific question definitions. | verified | `app/data/wizardDefinitions.ts`, `app/domain/wizard.ts` | `tests/unit/wizardDefinitions.test.ts`, `tests/unit/requestContract.test.ts` | 2 |
| FLOW-002 | A successful request submission opens one customer workspace with scope, current stage, next action, offers and job handoff. | verified | `app/components/RequestWizard.tsx`, `app/components/RequestScopeSummary.tsx`, `app/taleplerim/[id]/teklifler/page.tsx` | `tests/component/DirectedRequestWizard.test.tsx`, `tests/component/RequestScopeSummary.test.tsx`, `tests/unit/requestJourney.test.ts` | 2 |
...
| WIZ-005 | The wizard locks background scroll, restores modal focus and renders a reduced-motion-safe receipt only after authoritative submission success. | verified | `app/hooks/useModalDialog.ts`, `app/components/wizard/WizardSuccessReceipt.tsx`, `app/components/RequestWizard.tsx` | `tests/component/RequestWizard.test.tsx`, `tests/component/DirectedRequestWizard.test.tsx`, `tests/component/WizardSuccessReceipt.test.tsx`, `tests/e2e/home.spec.ts` | 2 |
