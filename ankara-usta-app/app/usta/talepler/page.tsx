...
export default async function TradespersonRequestsPage() {
  ...
  const { data, error } = await supabase
    ...
    .select('request_id,score,reasons,service_requests(service_id,district,neighborhood,preferred_timing,status)')
  ...
  return (
    <main className="account-shell requests-page">
      <AppHeader role="tradesperson" />
      <div style={{ maxWidth: '1140px', margin: '32px auto', padding: '0 16px' }}>
        <div className="requests-header">
          <Link className="account-back" href="/">← Ana Sayfa</Link>
          <div>
            <span>USTA İŞ HAVUZU</span>
            <h1>Eşleşen talepler</h1>
            <p style={{ margin: '4px 0 0' }}>
              <Link href="/usta/musaitlik" style={{ color: 'var(--action-info)', fontWeight: 650 }}>
                Müsaitliğinizi ve çalışma günlerinizi güncelleyin →
              </Link>
            </p>
          </div>
        </div>
        ...
        {error ? (
          <p className="account-message">Eşleşmeler yüklenemedi.</p>
        ) : matches.length ? (
              ...
              return (
                <article key={match.request_id}>
                  ...
                  {request?.status === 'provider_selected' ? (
                    ...
                    <Link className="dialog-primary" href={`/usta/teklifler/${match.request_id}`}>
                      Teklif oluştur
                    </Link>
                  )}
                </article>
          ...
          <section className="account-card empty-requests">
            <h2>Henüz eşleşen talep yok</h2>
            <p>Hizmet, bölge, doğrulama ve müsaitlik koşulları birlikte sağlandığında yeni talepler burada anlık olarak listelenir.</p>
          </section>
        )}
      </div>
    </main>
