...
export default async function CustomerQuotesPage({params}:{params:Promise<{id:string}>}){
  ...
  const [{data:request,error:requestError},{data:run,error:runError},{data:matchRows,error:matchError},{data:quoteRows,error}]=await Promise.all([
  ...
  return <main className="account-shell customer-quotes"><RealtimeRefresh channelName={`customer-quotes-${id}`} subscriptions={[{table:'service_requests',filter:`id=eq.${id}`},{table:'matching_runs',filter:`request_id=eq.${id}`},{table:'request_matches',filter:`request_id=eq.${id}`},{table:'quotes',filter:`request_id=eq.${id}`},...(direct&&directedRequestsEnabled()?[{table:'request_invitations',filter:`request_id=eq.${id}`}]:[])]} label="Teklifler"/><Link className="account-back" href="/taleplerim">← Taleplerim</Link><header><span>TEKLİF KARŞILAŞTIRMA</span><h1>{service?.name??request.service_id}</h1><p>{request.neighborhood}, {request.district}</p></header><RequestConversationLinks requestId={id}/>{direct&&(invitation?<RequestInvitationPanel key={invitation.status} invitation={invitation} serviceId={request.service_id} role="customer" canRespond={['submitted','matching','quotes_received'].includes(request.status)}/>:<p role="alert">Özel talebin yanıt durumu şu anda alınamıyor.</p>)}{!direct&&(runError||matchError)&&<p role="alert">Eşleştirme bilgileri yüklenemedi.</p>}{!direct&&run&&<section className={`supply-state ${run.supply_state}`}><b>{run.eligible_count} uygun usta</b><p>{supplyCopy[run.supply_state as keyof typeof supplyCopy]}</p></section>}{!direct&&<section className="match-explanations"><h2>Eşleştirme nedenleri</h2>{matched.map(item=><article key={`${item.tradesperson_profiles?.display_name}-${item.score}`}><b>{item.tradesperson_profiles?.display_name??'Usta'} · {item.score}/100</b><p>{item.reasons.join(' · ')}</p></article>)}</section>}{error?<section className="account-card account-state-error" role="alert"><h2>Teklifler yüklenemedi</h2><p>Veriler geçici olarak alınamadı. Sayfayı yenileyerek tekrar deneyin.</p></section>:quotes.length?<><div className="quote-profile-links">{(quoteRows as unknown as QuoteRow[]).map(quote=><Link key={quote.id} href={`/ustalar/${quote.tradesperson_id}`}>{quote.tradesperson_profiles?.display_name??'Usta'} profilini incele →</Link>)}</div><QuoteComparison quotes={quotes}/></>:<section className="account-card empty-requests"><h2>Henüz teklif yok</h2><p>{direct?'Seçtiğiniz ustanın gönderdiği teklif burada görünecek. Davetin yanıt durumunu yukarıdan takip edebilirsiniz.':'Uygun ustalar kapsamı inceleyip teklif gönderdiğinde burada en fazla üç teklifi yan yana karşılaştırabilirsiniz.'}</p></section>}</main>;
}
