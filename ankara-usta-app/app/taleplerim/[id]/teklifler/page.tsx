...
type QuoteRow={id:string;version:number;status:string;labor_amount_kurus:number;material_amount_kurus:number;estimated_duration_minutes:number;warranty_days:number;included_scope:string[];excluded_scope:string[];note:string|null;tradesperson_profiles:{display_name:string}|null};
...
export default async function CustomerQuotesPage({params}:{params:Promise<{id:string}>}){
  ...
  const [{data:request},{data:run},{data:matchRows},{data:quoteRows,error}]=await Promise.all([
    ...
    supabase.from('quotes').select('id,version,status,labor_amount_kurus,material_amount_kurus,estimated_duration_minutes,warranty_days,included_scope,excluded_scope,note,tradesperson_profiles(display_name)').eq('request_id',id).in('status',['submitted','accepted']).order('created_at',{ascending:true}),
  ]);
  if(!request)notFound();
  ...
  return <main className="account-shell customer-quotes"><Link className="account-back" href="/taleplerim">← Taleplerim</Link><header><span>TEKLİF KARŞILAŞTIRMA</span><h1>{service?.name??request.service_id}</h1><p>{request.neighborhood}, {request.district}</p></header>{run&&<section className={`supply-state ${run.supply_state}`}><b>{run.eligible_count} uygun usta</b><p>{supplyCopy[run.supply_state as keyof typeof supplyCopy]}</p></section>}<section className="match-explanations"><h2>Eşleştirme nedenleri</h2>{matched.map(item=><article key={`${item.tradesperson_profiles?.display_name}-${item.score}`}><b>{item.tradesperson_profiles?.display_name??'Usta'} · {item.score}/100</b><p>{item.reasons.join(' · ')}</p></article>)}</section>{error?<p className="account-message">Teklifler yüklenemedi.</p>:quotes.length?<QuoteComparison quotes={quotes}/>:<section className="account-card empty-requests"><h2>Teklifler bekleniyor</h2><p>Uygun ustalar kapsamı inceleyip teklif gönderdiğinde burada en fazla üç teklifi yan yana karşılaştırabilirsiniz.</p></section>}</main>;
}
