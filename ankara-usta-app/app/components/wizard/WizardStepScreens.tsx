...
function StepScreen({kicker,title,description,note,children}:StepScreenProps){
  return <div className="wizard-step-content animate-fade-in"><header className="step-content-header"><span className="step-kicker">{kicker}</span><h2 id="wizard-title" className="step-main-title">{title}</h2>{description&&<p className="step-sub-desc">{description}</p>}{note}</header>{children}</div>;
}
